...
async function yellowshoessConversation(conversation: Conversation<MyContext>, ctx: MyContext) {
  ...
  const image = await generateImage(`TOK ${message.text}` || "", "yellowshoess", ctx.from?.id.toString());
...
export { yellowshoessConversation };
