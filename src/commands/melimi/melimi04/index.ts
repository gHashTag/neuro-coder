...
const melimi04 = async (ctx: Context): Promise<void> => {
  try {
    ...
    const history = await getHistory("melimi", "melimi04", "reels")
    const slides = await getSlides({
      prompt: `Напиши текст на русском языке. Напиши реально интересный текст, который будет интересен школьницам. Напиши топ какие-то интересные научные факты, научпоп формат. ФАКТЫ НЕ ДОЛЖНЫ ПОВТОРЯТЬСЯ. История ответов: ${history}`,
      scenesCount: 1,
      isDescription: true,
    })
    console.log(history)
    ...
    await makeTextLayers(slides.reels.onScreenTitle, `${photoTempPath}/photo.png`, false, `${logoPath}/01.png`, "sciencepop")
    ...
    //await setHistory("melimi", slides.reels.videoDescription, videoUrl, "melimi04", "reels");
