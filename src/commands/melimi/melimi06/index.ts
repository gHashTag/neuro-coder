...
const melimi06 = async (ctx: Context): Promise<void> => {
  try {
    ...
    const history = await getHistory("melimi", "melimi06", "reels")
    const slides = await getSlides({
      prompt: `Напиши текст на русском языке. Напиши реально интересный текст, который будет интересен школьницам. Напиши интересные факты про школьную жизнь, шоу "А вы знали?". ИНФОРМАЦИЯ НЕ ДОЛЖНА ПОВТОРЯТЬСЯ. История фактов: ${history}`,
      scenesCount: 1,
      isDescription: true,
    })
    console.log(history)
    ...
    await makeTextLayers(slides.reels.onScreenTitle, `${photoTempPath}/photo.png`, false, `${logoPath}/01.png`, "didyouknow")
    ...
    //await setHistory("melimi", slides.reels.videoDescription, videoUrl, "melimi06", "reels");
