...
const melimi05 = async (ctx: Context): Promise<void> => {
  try {
    ...
    const history = await getHistory("melimi", "melimi05", "reels");
    const slides = await getSlides({
      prompt: `Напиши текст на русском языке. Напиши реально интересный текст, который будет интересен школьницам. Напиши полезные шпоргалки для школы, нужно чтоб они вмещали много информации на объём. ШПОРГАЛКИ НЕ ДОЛЖНЫ ПОВТОРЯТЬСЯ. История шпоргалок: ${history}`,
      scenesCount: 1,
      isDescription: true,
    });
    console.log(history);
    ...
    await makeTextLayers(slides.reels.onScreenTitle, `${photoTempPath}/photo.png`, false, `${logoPath}/01.png`, "cheatsheet");
    ...
    //await setHistory("melimi", slides.reels.videoDescription, videoUrl, "melimi05", "reels");
