...
const melimi02 = async (ctx: Context): Promise<void> => {
  try {
    ...
    const history = await getHistory("melimi", "melimi02", "reels");
    const slides = await getSlides({
      prompt: `Напиши текст на русском языке. Напиши реально интересный текст, который будет интересен школьницам. Напиши какую-то интересную информацию по математике. ИНФОРМАЦИЯ НЕ ДОЛЖНА ПОВТОРЯТЬСЯ. История ответов: ${history}`,
      scenesCount: 1,
      isDescription: true,
    });
    console.log(history);
    ...
    await makeTextLayers(slides.reels.onScreenTitle, `${photoTempPath}/photo.png`, false, `${logoPath}/01.png`, "math");
