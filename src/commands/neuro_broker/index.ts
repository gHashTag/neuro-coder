...
import fs from "fs";
import { bathroomDescription, bedroomDescription, description, diningRoomDescription, kitchenDescription, livingRoomDescription, location, officeDescription, outdoorDescription, stepsData, triggerWord, type } from "./mock";
      ...
      try {
        if (videoOutput && videoOutput.filePath) {
          ...
          if (lang !== 'zh' && lang !== 'ar') {
            ...
            await setHistory({ brand: "neuro_broker", response: `Video ${lang.toUpperCase()} NeuroBroker`, video_url: render[0].url, command: "neuro_broker", type: "video", voice_id: "PVKVligmzACf89A0Cegd", chat_id: ctx.from.id.toString(), lang: lang, trigger: triggerWord });
            await deleteFileFromSupabase("neuro_broker", fileName);
        } else {
          await ctx.replyWithVideo(new InputFile(videoOutput.filePath), {
            caption: `Video ${lang.toUpperCase()} NeuroBroker`,
          });
          await setHistory({ brand: "neuro_broker", response: `Video ${lang.toUpperCase()} NeuroBroker`, video_url: videoUrl, command: "neuro_broker", type: "video", voice_id: "PVKVligmzACf89A0Cegd", chat_id: ctx.from.id.toString(), lang: lang, trigger: triggerWord });
        }
          // return;
        } else {
