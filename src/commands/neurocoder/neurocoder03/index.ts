...
const neurocoder03 = async (ctx: Context): Promise<void> => {
  try {
    ...
    const themes = ["моды", "науки", "дизайна", "бизнеса", "школы"];
    const randomThemeIndex = Math.floor(Math.random() * themes.length);
    const theme = themes[randomThemeIndex];

    const history = await getHistory("neurocoder", "neurocoder03", "reels");
    const slides = await getSlides({
      prompt: `Напиши текст на русском языке. Напиши реально интересный текст, который будет интересен. Напиши топ 5 нейросетей для ${theme}. НЕЙРОСЕТИ НЕ ДОЛЖНЫ ПОВТОРЯТЬСЯ. История ответов: ${history}`,
      scenesCount: 1,
      isDescription: true,
    });
    console.log(history);
    ...
    await makeTextLayers(slides.reels.onScreenTitle, `${photoTempPath}/photo.png`, false, `${logoPath}/01.png`, `neurotop_${randomThemeIndex}`, 100);
    ...
    await setHistory("neurocoder", slides.reels.videoDescription, videoUrl, "neurocoder03", "reels", "", ctx.from.id.toString());
