...
async function vega_condominiumConversation(conversation: Conversation<MyContext>, ctx: MyContext) {
  ...
  const image = await generateImage(`TOK ${message.text}` || "", "vega_condominium", ctx.from?.id.toString());
...
export { vega_condominiumConversation };
