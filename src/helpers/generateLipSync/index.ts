...
export async function generateLipSync(videoUrl: string, audioUrl: string, userId: string) {
  ...
  const webhookUrl = isDev ? `${process.env.NGROK}/api/synclabs-webhook` : `${process.env.ELESTIO_URL}/api/synclabs-webhook`
...
export async function getVideoStatus(videoId: string) {
  const { data, error } = await supabase.from("synclabs_videos").select("*").eq("video_id", videoId).single()
